_.extend(Backbone.Validation.callbacks, {
	...
	invalid : function(view, attr, error, selector) {
		...
		if (control.data("error-style") === "tooltip") {
			...
			target.text(error);
		} else if (control.data("error-style") === "composite") {
			if (group.find(".error-composite").length === 0) {
				group.find(".control-composite").append("<div class=\"error-composite\"></div>");
			}
			
			var block = group.find(".error-composite");
			if (block.find("." + attr).length === 0) {
				block.append("<p class=\"help-block error-message " + attr + "\"></p>");
			}
			
			var target = block.find("." + attr);
			target.text(error);
		} else {
